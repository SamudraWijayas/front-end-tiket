...
const BottomNav = (props: PropTypes) => {
  ...
  return (
    <div className="fixed bottom-4 left-1/2 z-50 w-[92%] max-w-md -translate-x-1/2 rounded-2xl bg-white/90 shadow-lg backdrop-blur-md lg:hidden">
        ...
        {isAuthenticated ? (
          <Fragment>
            <Link
              href="/transaction"
              className={`flex flex-col items-center gap-1 transition-all duration-300 ${
                pathname === "/transaction" ? "text-blue-600" : "text-gray-500 hover:text-blue-500"
              }`}
            >
              <FileText
                size={24}
                className={`transition-transform duration-300 ${
                  pathname === "/transaction" ? "scale-110 stroke-blue-600" : "stroke-gray-500"
                }`}
              />
              <span
                className={`text-xs font-medium ${
