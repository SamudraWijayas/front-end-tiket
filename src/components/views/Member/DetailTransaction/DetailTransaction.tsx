...
import { MapPin, QrCode, Download, ChevronDown } from "lucide-react";
import { Divider, Skeleton, Card, Button, Spinner } from "@heroui/react";
      ...
      {loading ? (
        <div className="relative h-[500px]">
          <div className="absolute inset-0 z-[200] flex items-center justify-center ">
            <Spinner classNames={{ label: "text-foreground" }} variant="wave" />
          </div>
        </div>
      ) : (
            ...
            {dataTransaction?.status === "completed" && (
              <div>
                ...
                <div className="grid gap-6 sm:grid-cols-2">
                  {dataBarcode?.map(
                    (barcode: { code: string; _id: string }) => (
                      <Skeleton
